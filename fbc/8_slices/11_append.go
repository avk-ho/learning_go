...
func getCostsByDay(costs []cost) []float64 {
	...
	for _, current_cost := range costs {
		current_day := current_cost.day
		current_value := current_cost.value
		if current_day >= len(CostsByDay) {
			for current_day >= len(CostsByDay) {
				CostsByDay = append(CostsByDay, 0.0)
			}
		}
		CostsByDay[current_day] += current_value
	}
